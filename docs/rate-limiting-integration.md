# rate limiting - Integration Notes
...
### Integration Points
1. **Backend Server** (`server.js`): Add route handler for /rate-limiting
...
### Rollback Plan
If issues arise, disable via `rate-limiting_ENABLED=false` environment variable.
