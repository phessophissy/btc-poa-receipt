# receipt timestamp tz - Integration Notes
...
### Integration Points
1. **Backend Server** (`server.js`): Add route handler for /receipt-timestamp-tz
...
### Rollback Plan
If issues arise, disable via `receipt-timestamp-tz_ENABLED=false` environment variable.
