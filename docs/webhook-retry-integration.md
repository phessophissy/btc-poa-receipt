# webhook retry - Integration Notes
...
### Integration Points
1. **Backend Server** (`server.js`): Add route handler for /webhook-retry
...
### Rollback Plan
If issues arise, disable via `webhook-retry_ENABLED=false` environment variable.
