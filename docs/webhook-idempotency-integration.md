# webhook idempotency - Integration Notes
...
### Integration Points
1. **Backend Server** (`server.js`): Add route handler for /webhook-idempotency
...
### Rollback Plan
If issues arise, disable via `webhook-idempotency_ENABLED=false` environment variable.
