# webhook handler - Integration Notes
...
### Integration Points
1. **Backend Server** (`server.js`): Add route handler for /webhook-handler
...
### Rollback Plan
If issues arise, disable via `webhook-handler_ENABLED=false` environment variable.
