# session persistence - Integration Notes
...
### Integration Points
1. **Backend Server** (`server.js`): Add route handler for /session-persistence
...
### Rollback Plan
If issues arise, disable via `session-persistence_ENABLED=false` environment variable.
